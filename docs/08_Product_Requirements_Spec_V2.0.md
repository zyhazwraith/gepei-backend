...
2.  **支付超时**: 所有“待支付”状态订单，创建后 **60分钟** 内未支付自动关闭 (系统保留15分钟缓冲期)。
3.  **退款政策 (风控)**:
    *   **冷静期 (1小时内)**: 下单支付后1小时内申请退款，**全额退款**。
    *   **违约期 (超过1小时)**: 扣除 **¥150** 违约金，退还剩余金额。
    *   **执行**: 用户在App订单详情页**自助发起** -> 系统自动计算金额并原路退回。
4.  **支付一致性约束**:
    *   使用 `transactionId` 作为支付链路统一标识。
    *   同一业务单（订单/加时）仅允许一条 payment 记录，重复发起支付复用该记录。
    *   微信授权采用“入站授权一次 + 会话复用 `openid`”策略；支付接口在会话失效时才触发重新授权。
...
【后台操作】
客服在后台"创建定制订单"
(录入：客户手机号、需求详情、**地陪手机号**、**小时单价(分)、预估时长**)
  ↓
【App端同步】
用户收到通知 / 刷新"我的订单"列表
  ↓
...
【后台审核与状态管理】(V2.2 核心变更)
  ↓
管理员审核通过 -> [isGuide=true] (获得认证资格)
  ↓
【地陪自主管理展示】
地陪/管理员可在详情页切换 [展示状态] (Status: Online/Offline)
  - 只有 [isGuide=true] 才能设为 [Online]
  - 设为 [Online] 后，才会出现在C端列表
  ↓
结束
```
...
- **状态分离 (V2.2)**: 审核通过不等于自动展示。引入 `status` 字段控制上下架。
...
- **操作区**:
    ...
    - **审核决议**: [通过] (设置 `isGuide=true`) / [驳回]。
    - **展示状态 (V2.2)**: 开关 [Online/Offline]。开启前提是已通过审核。
    - **后台代录 (V2.2)**: 客服可点击“创建地陪”，为指定用户录入全套资料并设置状态。
- **封禁管理**:
    - [封禁账号]: 需填写封禁原因。
    - [解禁账号]: 恢复正常状态。
...
### 流程12：用户自助退款 (User Auto-Refund) `[V2.1 新增]`
1.  **触发**: 用户在订单详情页（状态为 `待服务`）点击 "申请退款"。
2.  **系统判定**:
    *   **计算时长**: `当前时间 - 支付时间`。
    *   **规则 A (<1h)**: 提示 "当前退款免费，资金将全额原路返回"。
    *   **规则 B (>1h)**: 提示 "距离支付已超过1小时，申请退款将扣除 ¥150 违约金"。
3.  **执行**:
    *   用户确认退款。
    *   系统调用微信支付退款接口。
    *   订单状态变更为 `REFUNDED`。
    *   记录退款流水（含微信退款单号）。
4.  **结果**: 界面展示 "已退款"，资金预计1-3个工作日到账。
...
*   **状态机 (State Machine)**:
    1.  **待支付 (WAITING_PAYMENT)**: 初始状态。**60分钟**未支付自动转为`CANCELLED` (系统缓冲15分钟)。
    ...
    7.  **已退款 (REFUNDED)**: 待服务订单被执行退款。
*   **财务与视角模型** `[V2 补充]`:
    *   **金额定义**:
        *   **订单原价 (Amount)**: 初始下单时的基础费用（订金或全款），不含加时费。
        *   **总流水 (Total Amount)**: `原价 + 累积已付加时费`。用于展示用户总支出。
        *   **地陪收入 (Guide Income)**: `总流水 * (1 - 平台抽成率)`。数据持久化存储，不随费率变动而改变。
    *   **双视角隔离 (View Isolation)**:
        *   **买家视角 (Customer View)**: 展示用户作为消费者预订的订单。*需严格过滤掉用户作为地陪接的单（防止测试数据混淆）*。
        *   **卖家视角 (Guide View)**: 展示用户作为地陪接到的订单。金额显示为“本单收入”（红色高亮）。
*   **加时计算规则** `[V2 补充]`:
...
#### 5.3.1 订单管理 (Order Management) `[V2 Enhanced]`
*   **筛选与搜索能力**:
    *   **多状态组合筛选 (Multi-Status)**: 支持按业务场景聚合筛选，前端 Tab 传递逗号分隔字符串，后端支持 `IN` 查询。
    *   **混合关键词搜索 (Hybrid Search)**: 搜索框支持同时匹配 **订单号**、**用户手机号**、**客服(创建人)手机号**、**地陪手机号/花名**。
*   **信息展示规范**:
    *   **字段标准化**: 涉及人员姓名的字段统一为 `xxxName` (如 `userName`, `creatorName`, `guideName`)，废弃 `Nickname` 混用。
    *   **关键信息透出**: 列表页必须直观展示订单的 **创建人** (溯源是哪个客服建的单) 和 **地陪** (指派给了谁)。
*   **操作约束**:
    *   **禁止随意改状态**: 废弃 V1 的“强制修改状态”功能。状态流转必须通过具体的业务动作触发（如：指派地陪 -> 状态变更为待服务；执行退款 -> 状态变更为已退款）。
