...
1.  **触发**: 用户在订单详情页（状态为 `待接单` 或 `待服务`）点击 "申请退款"。
2.  **系统判定**:
    *   **计算时长**: `当前时间 - 支付时间`。
...
*   **筛选与搜索能力**:
    *   **多状态组合筛选 (Multi-Status)**: 支持按业务场景聚合筛选，例如“待服务”可同时包含 `Paid` (已支付) 和 `Waiting Service` (已指派)。前端 Tab 传递逗号分隔字符串，后端支持 `IN` 查询。
