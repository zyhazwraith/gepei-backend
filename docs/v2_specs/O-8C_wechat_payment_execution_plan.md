...
3. `queryAndSyncByTransactionId(...)`（主动查询并触发确认）
...
### 2.3 主动查询触发来源
1. 状态查询接口直接触发：`GET /api/v1/payments/:transactionId/status`
2. 定时任务触发：`jobs` 模块扫描 pending 并调用 `queryAndSyncByTransactionId(...)`

## 3. 统一数据约束
1. `payments` 是统一支付主表，订单与加时都写入该表。
2. 通过 `relatedType=order|overtime`、`relatedId` 分流业务。
3. `transactionId` 为统一查询主键（必须唯一）。
4. 本期直接复用 `payments.transactionId` 作为统一交易号，不新增 `outTradeNo` 字段。

## 4. 运行流程（简化）
### 4.1 预支付
1. 前端请求订单/加时支付接口，传 `authCode`。
2. 后端解析 OpenID。
3. 创建本地 payment（`pending` + `transactionId`）。
...
### 4.3 主动查询
1. 状态查询接口按 `transactionId` 查本地状态。
2. 若本地为 `pending`，调用 `queryAndSyncByTransactionId(...)` 查微信并同步。
...
1. `POST /api/v1/orders/:id/payment`
   ...
   - 行为：创建预支付，返回 `transactionId + payParams`
2. `POST /api/v1/overtime/:id/pay`
   - 入参：`paymentMethod`, `authCode`
   - 行为：创建预支付，返回 `transactionId + payParams`
3. `GET /api/v1/payments/:transactionId/status`
   - 行为：查询本地状态；若 pending，则触发一次 `queryAndSyncByTransactionId(...)`
...
3. `GET /api/v1/payments/:transactionId/status` 可触发 mock 查单并同步状态。
...
2. `GET /api/v1/payments/:transactionId/status` 接入真实查单结果。
...
4. 主动查询函数：`queryAndSyncByTransactionId`
...
## 7. 测试点（结构化）
### UT
1. 预支付创建：返回 `transactionId/payParams`，业务状态不变。
2. `confirmPaid` 成功：订单/加时分流正确。
3. 幂等：同一 `transactionId` 重复确认仅一次生效。
4. 金额校验失败：拒绝迁移，状态保持不变。
5. `queryAndSyncByTransactionId` 在查单成功时触发 `confirmPaid`。
...
5. `GET /payments/:transactionId/status` 可触发一次主动查询。
6. `jobs` 补偿任务可补齐 pending 成功单。

## 8. 简化策略（小团队）
1. 本期不做复杂失败终态编排，优先保证成功确认链路正确。
2. 本期先不做额外查询限流策略（后续按监控补）。
3. 仍需保留安全底线：验签、金额校验、幂等、唯一 `transactionId`。
