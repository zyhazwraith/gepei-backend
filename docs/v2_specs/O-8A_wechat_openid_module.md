...
## 3. 业务规则
1. 首次进入微信内页面时，前端触发静默 OAuth 获取 `authCode`。
2. 后端将 `authCode` 换取 `openid` 后，绑定到“当前登录会话”（内存版）。
3. 支付下单优先复用会话 `openid`，不要求每次支付都携带 `authCode`。
4. 若会话未绑定或授权失效，返回 `WECHAT_REAUTH_REQUIRED`，前端重走一次 OAuth 后重试。
5. `authCode` 为一次性短时凭证，不做长期缓存；`openid` 为会话级可复用凭证。
6. 不在 `users` 表做永久强绑定（避免手机号账号切换微信时被锁死）。
...
## 5. API 设计（对前端）
OpenID 模块新增会话绑定接口，支付接口按会话复用。

### 5.1 会话绑定接口
`POST /api/v1/payments/wechat/session-openid`
...
1. 校验登录态
2. `authCode -> openid`
3. 按 `userId + token 指纹` 绑定会话 `openid`（内存）
4. 记录审计日志（`openidHash`）

### 5.2 支付接口
`POST /api/v1/orders/:id/payment` / `POST /api/v1/overtime/:id/pay`
...
后端处理顺序:
1. 验证订单与支付资格
2. 若请求带 `authCode`：实时换取并覆盖会话 `openid`
3. 否则读取当前会话 `openid`
4. 无会话 `openid` 时返回 `WECHAT_REAUTH_REQUIRED`
5. 把 `openid` 传入支付模块下单
...
5. 会话绑定成功后，不带 `authCode` 也可预支付

集成测试:
1. `POST /payments/wechat/session-openid` 可成功绑定会话 `openid`
2. 未绑定会话时，不带 `authCode` 下单返回 `WECHAT_REAUTH_REQUIRED`
3. 绑定成功后，不带 `authCode` 下单可成功创建预支付
4. `authCode` 无效时支付流程中止，不创建预支付单
...
## 11. 已确认决策
1. `authCode` 不做长期缓存，使用后即弃。
2. `openid` 做会话级复用（内存版），不做用户永久强绑定。
3. 会话键使用 `userId + token 指纹`，减少同账号多设备串用风险。
4. 失败后前端自动重走一次 OAuth，若仍失败再提示手动重试。
5. 通过安全日志记录 `openidHash` 做联调与审计。
