...
| **avatar_id** | INT | FK(attachments) | 头像附件ID | **[修改]** 存 ID |
...
| guide_id | INT | FK(users) | 接单地陪 (关联 guides.user_id) | **[修改]** 外键指向 users |
...
| **amount** | INT | | 订单总额 (单位: 分) | **[修改]** 改用 INT |
| **refund_amount** | INT | Default 0 | 退款金额 (单位: 分) | **[新增]** |
| **content** | JSON/TEXT | | **核心服务内容** (定制单需求) | **[修改]** |
...
*   `withdraw_unfreeze`: 提现驳回解冻 (Withdrawal Failed) -> 余额增加
...
| operator_id | INT | FK(users) | 操作人 (客服) | |
| created_at | TIMESTAMP | | | |
...
| time | TIMESTAMP | | 打卡时间 | |
| location | VARCHAR(255) | | 打卡地点 (LBS信息) | |
