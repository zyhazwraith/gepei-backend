...
| **status** | ENUM | 'online', 'offline' | **展示状态** (默认 offline) | **[V2.2 新增]** 控制C端可见性 |
| id_verified_at | TIMESTAMP | | 实名认证时间 | |

**业务逻辑说明**:
*   `users.isGuide`: 代表**认证资格** (Qualified)。
*   `guides.status`: 代表**展示状态** (Visibility)。
*   **约束**: 只有 `isGuide=true` 的用户才能设置 `status='online'`。
...
| **status** | ENUM | pending, waiting_service, in_service, **service_ended**, completed, cancelled, **refunded** | 订单状态 | **[修改]** |
...
| transaction_id | VARCHAR(64) | **Not Null, Unique** | 商户侧统一交易号 (`transactionId`) | **[修改]** 作为支付主查询键 |
...
**关键约束**:
* `UNIQUE(transaction_id)`：保证交易号全局唯一。
* `UNIQUE(related_type, related_id)`：保证同一业务单仅有一条 payment 记录。
...
**关键约束**:
* `UNIQUE(out_refund_no)`：保证商户退款单号全局唯一。
* `UNIQUE(order_id)`：保证单订单仅允许一条退款记录（一单一退款）。
* `status` 默认值为 `pending`，退款终态由回调/查单确认写入。
