...
*   Update `refund_records` table (Enhanced for Audit/Finance):
    *   `out_refund_no`: Unique refund request ID generated by system (e.g. `REF_ORD123_TIMESTAMP`).
    *   `refund_transaction_id`: Transaction ID returned by Payment Provider (e.g. WeChat Refund ID).
    *   `status`: `success` (default for synchronous flow), `pending`, `failed`.
    *   `amount`: Calculated amount.
    *   `reason`: "User Auto Refund (Penalty: [Yes/No])".

### 3.3 Payment Provider
*   Interface: `IPaymentProvider`
    *   Method `refund(...)` must return `refundTransactionId`.
*   Mock Implementation: `MockWechatProvider` (Simulate API call to WeChat Pay)
    *   Behavior: Log request, return success immediately with a mock `refundTransactionId`.

### 3.4 UI Guidelines
*   **Refund Button**: Placed inside the "Fee Details" card (not in the fixed bottom bar) to avoid occlusion and context switching.
