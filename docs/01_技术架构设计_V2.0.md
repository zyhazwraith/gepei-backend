...
- **统一支付主键**: 使用 `transactionId` 作为支付链路唯一标识（预支付、查单、回调、状态查询统一使用）。
- **单业务单唯一 payment**: 同一 `(related_type, related_id)` 仅允许一条支付记录，重复发起支付时复用该记录。
- **预支付编排**: `createPrepay` 采用“先插入，唯一冲突再查找”的路径，减少并发下分支复杂度。
- **OpenID 会话绑定**: 前端首次进入微信内页面获取 `authCode` 后，调用后端绑定会话 `openid`；支付接口优先复用会话 `openid`，仅在缺失/失效时要求重授权。
- **会话隔离键**: 会话 `openid` 绑定按 `userId + token指纹` 存储（内存版），避免同账号多端串用。
- **支付结果来源**: 支付成功确认统一由三条路径触发：微信 `notify`、状态查询接口触发主动查单、定时任务补偿查单。
- **幂等与补偿**: `payment CAS` + `业务 CAS` 双层幂等；定时任务补偿 pending 查单与 success 未落账场景。
- **审计日志**: `openid` 绑定仅记录 `openidHash`、会话键、用户ID、IP、UA，不记录明文 `openid`。
- **退款机制**: 
    - **人工介入模式**: 系统不设自动退款规则引擎。
