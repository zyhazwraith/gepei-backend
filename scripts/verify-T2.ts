...
async function main() {
  try {
    ...
    const payload = {
      userPhone: USER_PHONE, // Ensure this user exists too
      guideId: guideId,
      pricePerHour: 100.5, // 100.5 Yuan
      duration: 8,         // 8 Hours
    ...
    // Assertions
    if (data.amount !== 80400) throw new Error(`Amount mismatch: expected 80400, got ${data.amount}`); // 100.5 * 100 * 8
    if (data.status !== 'pending') throw new Error(`Status mismatch: expected pending, got ${data.status}`);
    console.log('✅ Success Case Passed');

    // 4. Test Error: Missing Guide
    console.log('\n4. Testing Error: Missing GuideId...');
    try {
        const invalidPayload = { ...payload };
        delete (invalidPayload as any).guideId;
        await axios.post(`${API_URL}/admin/custom-orders`, invalidPayload, {
            headers: { Authorization: `Bearer ${token}` }
        });
        console.error('❌ Failed: Should have returned 400 for missing guideId');
    } catch (e: any) {
        if (e.response?.status === 400) console.log('✅ Error Case Passed (400)');
        else console.error(`❌ Unexpected Error: ${e.message}`);
    }

    // 5. Test Error: Invalid Price
    console.log('\n5. Testing Error: Negative Price...');
    try {
        await axios.post(`${API_URL}/admin/custom-orders`, { ...payload, pricePerHour: -10 }, {
            headers: { Authorization: `Bearer ${token}` }
        });
        console.error('❌ Failed: Should have returned 400 for negative price');
    } catch (e: any) {
        if (e.response?.status === 400) console.log('✅ Error Case Passed (400)');
        else console.error(`❌ Unexpected Error: ${e.message}`);
    }
